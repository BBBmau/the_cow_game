...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        res.end(JSON.stringify({ 
            ...
            redis: redisHelpers.isConnected() ? 'connected' : 'disconnected'
            }));
            return;
        }
...
// Start server first so the process stays up and becomes Ready even if Redis is slow/missing.
// Initialize Redis in the background so stats work when Redis is available.
async function startServer() {
    ...
    // Redis in background so startup is not blocked; stats work when Redis is available
    initializeRedis().catch((err) => console.error('Redis init (background):', err));
}
